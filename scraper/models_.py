...
class Zaak:
    ...
    # :beschrijving (OData "Onderwerp")
    beschrijving: Optional[str] = field(default=None)
    indienings_datum: Optional[datetime] = field(
        default=None)  # :indieningsDatum (OData "GestartOp")
    termijn: Optional[datetime] = field(
        default=None)      # :termijn (End date)
    besluit_resultaat: Optional[str] = field(default=None)  # :besluitResultaat
    besluit_stemming_soort: Optional[str] = field(
        default=None)  # :besluitStemmingSoort
    is_afgedaan: bool = field(default=False)               # :isAfgedaan
    kabinets_appreciatie: Optional[str] = field(
        default=None)  # :kabinetsappreciatie

    # Object Properties (Relationships)
    onderwerpen: List[Onderwerp] = field(
        default_factory=list)  # :heeftOnderwerp
    stemmingen: List['Stemming'] = field(
        default_factory=list)  # :heeftStemming
...
class Stemming:
    ...
    """
    # Datatype Properties
    stemming_soort: Optional[str] = field(
        default=None)              # :stemmingSoort
    fractie_groote_op_moment_van_stemming: Optional[int] = field(
        default=None)  # :fractieGrooteOpMomentVanStemming

    # Object Properties (Relationships)
    is_stemming_over: Zaak = field()                  # :isStemmingOver
    # :isUitgebrachtDoor (Inverse of :heeftGestemdOp)
    uitgebracht_door: Actor = field()
...
class Persoon(Actor):
    ...
    # Object Properties (Relationships)
    is_lid_van: Optional['Fractie'] = field(
        default=None)     # :isLidVan (Range is Fractie)
...
def main():
    ...
    print(
        f'Lid: {lid_diederik.naam}, Lid van: {lid_diederik.is_lid_van.afkorting}')
    print(f'Zaak (Motie): {motie.titel}')
    print(
        f'Stemming 1: Door {stemming_voor.uitgebracht_door.naam} ({stemming_voor.stemming_soort})')
